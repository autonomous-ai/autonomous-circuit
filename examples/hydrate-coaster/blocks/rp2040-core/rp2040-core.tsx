 ...
 * C4-C16.
 *
 * PROJECT-LOCAL PLACEMENT FIX (hydrate-coaster, 2026-08-10) — electrically
 * identical to the shared block; only pcbX/pcbY moved. As shipped, Y1 sat at
 * pcbX=-11 while XIN/XOUT are on the QFN's BOTTOM edge, putting Y1.pin1
 * 11.78mm from U3.XIN. tscircuit enforces DEFAULT_CRYSTAL_MAX_TRACE_LENGTH_MM
 * = 10 on every crystal net, so the autorouter SKIPPED THE WHOLE BOARD and
 * every trace came back missing — the block is unroutable on any board as
 * shipped (reproduced standalone). Y1/C15/C16/R11 are moved into the empty
 * slot directly below the chip, between SW2 and SW3; the longest crystal-net
 * hop is now 8.44mm, and the load caps sit beside the pins they serve, which
 * is what the RP2040 design guide asks for anyway. The upstream fix belongs
 * in packages/golden-blocks/blocks/rp2040-core.
 */
  ...
  return (
    <group pcbX={props.pcbX ?? 0} pcbY={props.pcbY ?? 0} schX={props.schX ?? 0} schY={props.schY ?? 0}>
      ...
      <crystal name={y} frequency="12MHz" loadCapacitance="10pF" pinVariant="four_pin"
        footprint="crystal" pcbX={0} pcbY={-11} schX={-14} schY={6}
      ...
      <resistor name="R11" resistance="1k" footprint="0402" pcbX={3} pcbY={-7.5} schX={-10} schY={7}
      ...
      <capacitor name="C15" capacitance="15pF" footprint="0402" pcbX={-3} pcbY={-9.5} schX={-16} schY={9}
        schRotation="90deg" supplierPartNumbers={{ jlcpcb: ["C1548"] }} />
      <capacitor name="C16" capacitance="15pF" footprint="0402" pcbX={3} pcbY={-9.5} schX={-12} schY={9}
        schRotation="90deg" supplierPartNumbers={{ jlcpcb: ["C1548"] }} />
