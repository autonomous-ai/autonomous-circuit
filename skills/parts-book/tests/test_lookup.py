...
from test_parts_book import _make_project  # noqa: E402
...
def test_lookup_fills_stock_price_and_basic(tmp_path: Path, monkeypatch):
    ...
    record = json.loads((project / "parts.json").read_text())["U9"]
...
def test_lookup_uses_the_on_disk_cache(tmp_path: Path, monkeypatch):
    ...
    assert len(fake.calls) == 6, "--no-cache refetches once per LCSC identity"
...
def test_partial_failure_keeps_the_parts_that_resolved(tmp_path: Path, monkeypatch):
    ...
    checked = {p["ref"]: p["stock_checked"] for p in payload["parts"]}
    assert checked["R90"] is None
    assert checked["R91"] is None
