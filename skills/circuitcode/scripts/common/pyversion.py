...
resolves — on some machines that is 3.9 while circuitpy requires >=3.10.
Call ensure_python() first thing in every __main__ shim: on an old
interpreter it searches (in order) $CIRCUIT_PYTHON, then python3.13..3.10 on
...
MIN = (3, 10)
_SENTINEL = "CIRCUIT_PYVERSION_REEXEC"
...
def ensure_python() -> None:
    ...
    if os.environ.get(_SENTINEL):
        sys.stderr.write(
            f"circuitcode: re-exec loop — {sys.executable} is still "
            f"{sys.version_info.major}.{sys.version_info.minor}\n"
        )
        raise SystemExit(2)

    candidates = []
    env_python = os.environ.get("CIRCUIT_PYTHON")
    if env_python:
    ...
    sys.stderr.write(
        f"circuitcode: needs python >= {MIN[0]}.{MIN[1]} but running "
        f"{sys.version_info.major}.{sys.version_info.minor} and no newer "
        "python3.x found on PATH (set CIRCUIT_PYTHON to override)\n"
    )
    raise SystemExit(2)
