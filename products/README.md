...
| `i2c-sensor-hub` | USB-C board that reads a BME280 and breaks its I2C bus out to a pad he | **yes** | 0 | **yes** | 95 × 67mm |
| `macropad-6` | USB-C six-key macropad: 3x2 grid of tactile switches on 19 | no | 10 | no (10) | 66 × 84mm · 6× drc_violation, 3× pcb_via_trace_clearance_error |
| `pixel-badge` | a wearable conference badge: 8 addressable RGB pixels across the top,  | no | 9 | no (9) | 82 × 58mm · 6× drc_violation, 2× pcb_via_trace_clearance_error |
| `rgb-lamp-controller` | USB-C desk lamp controller: RP2040 brain driving 8 on-board WS2812 pix | **yes** | 0 | **yes** | 84.9 × 68.6mm |
| `sensor-node-mini` | Small always-on environment node: temperature, humidity and pressure o | **yes** | 0 | **yes** | 80.9 × 45.9mm · ⚠ source is newer than this verdict |
...
**Verdict older than its own source**: `desk-air-monitor`, `sensor-node-mini` — the board was edited and never rebuilt, so its numbers describe a board that no longer exists (ledger #42).
