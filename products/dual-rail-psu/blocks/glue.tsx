...
export const PadHeader = (props: {
   ...
   *
   * Added for dual-rail-psu (2026-08-17) — `PadHeader` is where a rail's
   * *only* copper ends on a board with no MCU to hand it off to, and it had
   * no way to ask for more than the board-wide default. Per
   * docs/architecture/rail-width.md: a `<trace thickness="…">` reaches
   * `tscircuit`'s router as a per-net `nominalTraceWidth` (one declaration
   * anywhere on the net sets the whole net) and widens what the router
   * searches for rather than stamping copper on afterwards — but declaring it
   * blind has scrapped a board before, so measure the placement's ceiling
   * with `circuitpy.netwidth` first and pass only what it clears.
   */
