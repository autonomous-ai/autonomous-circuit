"""Source identity for episode generators — the donor ``source_hash``
algorithm (cadpy) ported verbatim to drama projects.

``episode_source_hash(script_path, project_root)`` hashes the entry file
(``source_hash``) and folds a whole-graph fingerprint over the entry plus
``series.py`` plus every local import discovered statically (``ast``-walked,
breadth-first). Editing the series bible therefore invalidates every
episode's fingerprint (contract §1: ``source.fingerprint``).

Differences from the donor, all deliberate:
  * search paths are the project root and the entry file's directory
    (drama projects have no repo-level CAD roots);
  * ``series.py`` is always seeded into the graph even if the entry never
    imports it — the contract says the fingerprint folds it unconditionally;
...
def episode_source_hash(script_path: Path, project_root: Path) -> PythonSourceHash:
    """Hash an episode generator plus local Python imports (and series.py)."""
    resolved_script = script_path.expanduser().resolve()
    resolved_root = project_root.expanduser().resolve()
    search_paths = _dedupe_paths([resolved_root, resolved_script.parent])
    allowed_roots = tuple(search_paths)

    files: dict[Path, str] = {}
    queue: list[Path] = [resolved_script, resolved_root / "series.py"]
    seen: set[Path] = set()
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                dependencies.update(
                    _resolve_module(str(alias.name or ""), search_paths, allowed_roots)
                )
        elif isinstance(node, ast.ImportFrom):
            dependencies.update(
                _resolve_import_from(node, file_path, search_paths, allowed_roots)
            )
...
def _resolve_import_from(
    ...
    if int(node.level or 0) > 0:
        ...
        dependencies.update(
            _resolve_module_from_directory(base_dir, module_name, allowed_roots)
        )
        for alias in node.names:
            ...
            dependencies.update(
                _resolve_module_from_directory(base_dir, child_name, allowed_roots)
            )
        return dependencies
...
def _resolve_module_from_directory(
    ...
    if _trackable_python_file(leaf_module, allowed_roots):
        return _package_init_files(base_dir, parts[:-1], allowed_roots) | {
            leaf_module.resolve()
        }
...
def _package_init_files(
    base_dir: Path, package_parts: list[str], allowed_roots: tuple[Path, ...]
) -> set[Path]:
    files: set[Path] = set()
