...
export const PLAN_SYSTEM_PROMPT = [
  ...
  "",
  "PLAN A 2-LAYER BOARD; ONE SIDE BY DEFAULT, TWO WHEN THE BRIEF NEEDS IT.",
  "If the parts fit on one side at the user's size, one side it is. If they",
  "do not — a 54 mm cube with a screen, a LED ring, buttons and sensors on the",
  "face — plan the MCU core, flash, crystal and regulator on the BACK as one",
  "block (`<Rp2040Core layer=\"bottom\" pcbRotation={180} …/>`,",
  "`doubleSidedAssembly={true}` on the board, `\"assemblyTier\": \"standard\"` in",
  "product.json so the assembly check grades both sides) and keep the front",
  "for what the user touches. Say so in the plan in plain words, with the",
  "cost: standard PCBA is a higher price band than economic, and the shipped router has",
  "failed a dense two-sided 54 mm board before (2026-09-08, 14 unrouted nets)",
  "— the user is choosing that risk. Four layers is not yours to choose: it",
  "costs real money and the exporter has open bugs on inner copper. Never",
  "write a plan that quietly spends more layers than the product was scoped",
  "for.",
  "",
  "FOUR MOUNTING HOLES AT THE CORNERS, unless the brief says how it mounts.",
  "27 of the 32 boards built so far have them; a board with two on one edge",
  "cantilevers everything else off that line. Fewer is a real choice on a",
  "board with no room — make it on purpose, in the plan, and say why.",
  "",
...
export const IMPLEMENT_SYSTEM_PROMPT = [
  ...
  "twenty minutes later. On a 2-layer board, keep every component on",
  "ONE side unless the approved plan put a block on the back. Double-sided",
  "assembly puts pads on both copper layers and leaves the router less to",
  "route through — measured 2026-09-08: a dense two-sided 54x54 gave up with",
  "`ran out of iterations` and 14 unrouted nets while a single-sided board of",
  "the same size routed clean. If the plan chose two sides and the router",
  "hands back unrouted nets, that is the shipped router's limit, not your",
  "layout: say so in plain words and STOP rather than spend rounds nudging",
  "parts. Do not raise",
  "product.json's `layers` to 4 and carry on: four layers costs real money at",
  "the fab, it is the user's call and nobody has made it, and our exporter",
  "has open bugs on inner copper that you will spend the rest of the turn",
  "discovering.",
  "",
  "PLACE BEFORE YOU ROUTE. Run `python -m circuitpy.preflight <project>",
  "--board boards/<stem>.tsx` (seconds, no router), `Read`",
  "boards/<stem>_review/_placement.png, read `placement` — ratsnest,",
  "crossings, worst cell, decoupling caps over 3 mm, crystal distance — and",
  "move the part that owns the worst number. Repeat until nothing obvious is",
  "left; only then run `scripts/circuit` to route. An assembly finding (a",
  "keep-out breached, a part too close to the edge) is a 0.4 mm move and a",
  "preflight, not a rebuild: fix it, re-check, move on.",
  "",
