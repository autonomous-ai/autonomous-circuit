...
// external store with `useSyncExternalStore` bindings so React can read
// snapshots without introducing a new runtime dependency.
//
...
// The fence the driver emits (as a text_delta) for both the AskUserQuestion
...
export function chatReducer(state, action, now = Date.now()) {
  switch (action.type) {
    ...
    case "remove_pending_attachment": {
      ...
      // The view-context note describes a frame that was attached; once the
...
export async function startTurn(userMessage, { attachments = [] } = {}, transport = getTransport()) {
  ...
  try {
    // The frame context (if any) goes to the model only — append it to the
    // message the backend sees, but keep `text` clean for the echoed bubble.
    // When a frame is sent with no instruction, prepend a directive asking
