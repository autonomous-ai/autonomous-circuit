...
test("startTurn dispatches the user turn", async () => {
  resetChatStore();
  setProject("proj-1");
  ...
  try {
    const result = await startTurn("make the hook land faster");
    assert.deepEqual(calls, [{ projectId: "proj-1", userMessage: "make the hook land faster" }]);
    ...
    assert.equal(state.history[0].userText, "make the hook land faster");
  } finally {
    restore();
    resetChatStore();
  }
