...
function ChatInput({ className, placeholder = "Describe a circuit board" }, ref) {
  ...
  // A turn needs text or at least one attached image.
  const sendDisabled =
    turnInProgress || (!value.trim() && !pendingAttachments.length);
  ...
  const handleSend = useCallback(async () => {
    ...
    if (!payload && !pendingAttachments.length) return;
    if (!currentProjectId) {
      try {
  ...
  useEffect(() => {
    ...
    window.addEventListener(PREFILL_CHAT_INPUT_EVENT, onPrefill);
    return () => window.removeEventListener(PREFILL_CHAT_INPUT_EVENT, onPrefill);
  }, []);
  ...
  const hasStrip = pendingAttachments.length > 0;
